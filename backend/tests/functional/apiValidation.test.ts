...
describe("API validation", () => {
  ...
  testCase(
    ...
    async () => {
      const { token } = await authenticateTestUser({ email: "schema@example.com", username: "schema" });
      const agent = request();
      const start = performance.now();
  ...
  testCase(
    ...
    async () => {
      const { token } = await authenticateTestUser({ email: "reco-schema@example.com", username: "reco-schema" });
      const agent = request();
      const start = performance.now();
  ...
  testCase(
    ...
    async () => {
      const agent = request();
      const start = performance.now();
      const response = await agent.post("/api/register").send({ email: "nouser@example.com", password: defaultPassword });
      const durationMs = performance.now() - start;
      expect(response.status).toBe(400);
