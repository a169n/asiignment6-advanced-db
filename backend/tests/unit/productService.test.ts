...
describe("productService", () => {
  ...
  it("supports text search and sorting", async () => {
    await createTestProduct({ productName: "Mountain Bike", description: "A bike for mountains", category: "sports", tags: ["bike"], rating: 4.7, popularity: 12 });
    await createTestProduct({ productName: "City Bike", description: "Urban commuting bike", category: "sports", tags: ["bike", "city"], rating: 4.2, popularity: 8 });
    await createTestProduct({ productName: "Trail Backpack", description: "Durable backpack", category: "outdoor", tags: ["bag"], rating: 4.9, popularity: 15 });
  ...
  it("filters by price range and category", async () => {
    ...
    const result = await findProducts({ category: "electronics", minPrice: 150, maxPrice: 500, sort: "priceAsc" });
